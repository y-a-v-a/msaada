...
async fn basic_file_serving() {
    ...
    setup_basic_test_files(&server.server_dir).await.expect("Failed to setup test files");

    // Test HTML file serving
    let response = client.get(&server.url_for("index.html")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let html_content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(html_content.contains("Welcome"), "HTML content should contain 'Welcome'");
    assert!(html_content.contains("<h1>"), "HTML should contain h1 tag");

    // Test CSS file serving
    let response = client.get(&server.url_for("style.css")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let css_content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(css_content.contains("background-color"), "CSS should contain background-color");

    // Test JavaScript file serving
    let response = client.get(&server.url_for("script.js")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let js_content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(js_content.contains("console.log"), "JS should contain console.log");

    // Test JSON file serving
    let response = client.get(&server.url_for("data.json")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let json_data: serde_json::Value = response.json_for_assertions().await.expect("Failed to parse JSON");
    assert_eq!(json_data["name"], "test", "JSON should contain correct name field");
    assert_eq!(json_data["value"], 42, "JSON should contain correct value field");
}
...
async fn content_types() {
    ...
    let content_type = response.headers().get("content-type")
        ...
        .to_str().expect("Content-Type should be valid string");
    assert!(content_type.starts_with("text/html"), "HTML should have text/html content type");
    ...
    let content_type = response.headers().get("content-type")
        .expect("Content-Type header should be present")
        .to_str().expect("Content-Type should be valid string");
    assert!(content_type.starts_with("text/css"), "CSS should have text/css content type");
    ...
    let content_type = response.headers().get("content-type")
        ...
        .to_str().expect("Content-Type should be valid string");
    assert!(
        content_type.starts_with("text/javascript") || content_type.starts_with("application/javascript"),
        "JS should have appropriate content type, got: {}", content_type
    );
    ...
    let content_type = response.headers().get("content-type")
        .expect("Content-Type header should be present")
        .to_str().expect("Content-Type should be valid string");
    assert!(content_type.starts_with("application/json"), "JSON should have application/json content type");

    // Test PNG content type
    let response = client.get(&server.url_for("test.png")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let content_type = response.headers().get("content-type")
        .expect("Content-Type header should be present")
        .to_str().expect("Content-Type should be valid string");
    assert!(content_type.starts_with("image/png"), "PNG should have image/png content type");
}
...
async fn response_headers() {
    ...
    let response = client.head(&server.url_for("/")).await.expect("HEAD request failed");
    response.assert_status(StatusCode::OK);

    let headers = response.headers();

    // Test for common HTTP headers that should be present
    assert!(headers.contains_key("content-length"), "Content-Length header should be present");
    assert!(headers.contains_key("date"), "Date header should be present");
...
async fn http_methods() {
    ...
    assert!(content.contains("Welcome"), "GET should return file content");

    // Test HEAD method (should return headers but no body)
    let response = client.head(&server.url_for("index.html")).await.expect("HEAD request failed");
    response.assert_status(StatusCode::OK);

    // HEAD should have Content-Length but empty body
    assert!(response.headers().contains_key("content-length"), "HEAD should include Content-Length");

    // Test OPTIONS method (implementation varies)
    let response = client.client.request(reqwest::Method::OPTIONS, server.url_for("/"))
        .send().await.expect("OPTIONS request failed");
    ...
    assert!(
        ...
        "OPTIONS should return appropriate status code, got: {}", status
    );
}
...
async fn error_handling() {
    ...
    // Test 404 Not Found
    let response = client.get(&server.url_for("nonexistent.html")).await.expect("GET request failed");
    response.assert_status(StatusCode::NOT_FOUND);

    // Test directory traversal protection
    let response = client.get(&server.url_for("../../../etc/passwd")).await.expect("GET request failed");
    let status = response.status();
    assert!(
        status == StatusCode::NOT_FOUND || status == StatusCode::FORBIDDEN,
        "Directory traversal should be blocked, got status: {}", status
    );

    // Test invalid URLs with null bytes
    let response = client.get(&format!("{}/%00%00invalid", server.url())).await.expect("GET request failed");
    let status = response.status();
    assert!(
        status == StatusCode::BAD_REQUEST || status == StatusCode::NOT_FOUND,
        "Invalid URLs should return appropriate error, got status: {}", status
    );
}
...
async fn special_files() {
    ...
    setup_special_test_files(&server.server_dir).await.expect("Failed to setup special files");

    // Test empty file
    let response = client.get(&server.url_for("empty.txt")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);
    ...
    let response = client.get(&server.url_for("large.txt")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let content = response.bytes().await.expect("Failed to get response bytes");
    assert!(content.len() > 1000000, "Large file should be approximately 1MB, got {} bytes", content.len());

    // Test hidden files (should be blocked)
    let response = client.get(&server.url_for(".hidden")).await.expect("GET request failed");
    let status = response.status();
    assert!(
        status == StatusCode::NOT_FOUND ||
        status == StatusCode::FORBIDDEN ||
        status == StatusCode::BAD_REQUEST,
        "Hidden files should be blocked, got status: {}", status
    );

    // Test files with spaces (URL encoded)
    let response = client.get(&server.url_for("file%20with%20spaces.txt")).await.expect("GET request failed");
    response.assert_status(StatusCode::OK);

    let content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(content.contains("Special content"), "File with spaces should return correct content");
}
...
async fn directory_index() {
    ...
    let content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(content.contains("Welcome"), "Root directory should serve index.html with Welcome message");
    ...
    let content = response.text_for_assertions().await.expect("Failed to get response text");
    assert!(content.contains("Welcome"), "Direct index.html access should work");
}
...
async fn setup_basic_test_files(server_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(server_dir.join("readme.txt"), "This is a plain text file for testing.")?;
